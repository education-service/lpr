...
public class CharsSegment {
	...
	public int charsSegment(final Mat input, Vector<Mat> resultVec) {
		...
		int w = input.cols();
		int h = input.rows();
		Mat tmpMat = new Mat(input, new Rect((int) (w * 0.1), (int) (h * 0.1), (int) (w * 0.8),
				(int) (h * 0.8)));
		...
		// 去除不在长宽比和面积限制内的块，也就是不符合车牌长宽比和面积的候选区域，将不符合特定尺寸的图块排除出去。
		...
		Vector<Rect> sortedRect = new Vector<>();
		// 对符合尺寸的图块按照从左到右进行排序
		...
		Vector<Rect> newSortedRect = new Vector<>();
	...
	private Boolean verifySizes(Mat r) {
		...
		return percPixels <= 1 && charAspect > minAspect && charAspect < maxAspect
