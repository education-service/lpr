...
public class PlateLocate {
	...
	public Vector<Mat> plateLocate(Mat src) {
		...
		GaussianBlur(src, src_blur, new Size(gaussianBlurSize, gaussianBlurSize), 0, 0,
				BORDER_DEFAULT);
		if (debug) {
		...
		// 找出候选车牌的轮廓，求出图中所有的轮廓；这个算法会把全图的轮廓都计算出来，因此要进行筛选。
		MatVector contours = new MatVector();
		findContours(img_threshold, contours, // 一个contours的向量
				CV_RETR_EXTERNAL, // 提取外部轮廓
				CV_CHAIN_APPROX_NONE); // 每个contours的所有像素值
		...
		for (int i = 0; i < rects.size(); i++) {
			...
			if (verifySizes(minRect)) {
				...
				// 由于新到的一批图片中发现有很多车牌是倾斜的，因此决定再次尝试这段代码。

				float r = minRect.size().width() / minRect.size().height();
				float angle = minRect.angle();
				Size rect_size = new Size((int) minRect.size().width(), (int) minRect.size()
						.height());
				if (r < 1) {
					angle = 90 + angle;
